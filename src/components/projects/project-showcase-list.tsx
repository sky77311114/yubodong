...
import SkillPill from "../skills/skills-pill";

export type ProjectShowcaseListItem = {
  index: number;
  title: string;
  href: string;
  tags: { name: string; icon: FC<SVGProps<SVGSVGElement>> }[];
  image: {
    LIGHT: string;
    DARK?: string;
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="max-w-xl text-base font-semibold text-muted-foreground sm:text-lg"></p>
        <div className="flex max-w-xl flex-wrap gap-1 text-accent-foreground">
          {props.data.tags.map((tag) => (
            <SkillPill key={tag.name} name={tag.name} icon={tag.icon} isTag />
          ))}
        </div>
      </div>
    </motion.div>
